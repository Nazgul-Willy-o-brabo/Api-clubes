...
let CodUs = document.getElementById('CodUs');
let labelCodUs = document.getElementById('label-CodUs');
let validCodUs = false;
...
CodUs.addEventListener('keyup', () => {
    if(CodUs.value.length <= 2){
        labelCodUs.setAttribute('style', 'color: red');
        labelCodUs.innerHTML = 'Codigo usuario *Insira no minimo 3 numeros';
        CodUs.setAttribute('style', 'border-color: red');
        validCodUs = false;

    } else {
       labelCodUs.setAttribute('style', 'color: green');
       labelCodUs.innerHTML = 'codigo usuario';
       CodUs.setAttribute('style', 'border-color: green');
       validCodUs = true;
    }
})
...
function cadastrar(){
    ...
    if(validNome && validUsuario && validSenha && validConfirmSenha && validCodUs ){
        fetch(URL, {
            method: 'POST',
            headers: {
